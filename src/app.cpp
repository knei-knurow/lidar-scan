...
App::App(std::vector<std::string>& args) {
    ...
    mode = static_cast<RPLIDARScanModes>(
        mode_temp % unsigned(RPLIDARScanModes::RPLIDAR_SCAN_MODES_COUNT));

    // Initialize objects
    cloud_ = std::make_unique<CloudCyl>();
    lidar_grabber_ =
        std::make_unique<RPLIDARPortGrabber>(port, DefaultBaudrate, mode, rpm);
...
void App::print_help() {
    std::cout << "Source:\thttps://github.com/knei-knurow/lidar-scan\n"
              ...
              << "\t-r --rpm \tRPLIDAR revolutions per minute ("
              << MinRPLIDARRPM << " - " << MaxRPLIDARRPM << ")\n"
...
bool App::get_flag(std::vector<std::string>& all_args,
    ...
    auto it = std::find_if(all_args.begin(), all_args.end(),
                           [flag_short, flag_long](const std::string& s) {
                               return s == flag_short || s == flag_long;
                           });
...
std::string App::get_argument(std::vector<std::string>& all_args,
    ...
    auto it = std::find_if(all_args.begin(), all_args.end(),
                           [arg_short, arg_long](const std::string& s) {
                               return s == arg_short || s == arg_long;
                           });
