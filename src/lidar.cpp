...
RPLIDARPortGrabber::RPLIDARPortGrabber(std::string portname,
    ...
    if (rpm < MinRPLIDARRPM || rpm > MaxRPLIDARRPM) {
        std::cerr << "lidar-scan: error: RPM value should be between "
                  << MinRPLIDARRPM << " and " << MaxRPLIDARRPM << std::endl;
...
bool RPLIDARPortGrabber::print_info() {
    ...
    if (IS_FAIL(res)) {
        std::cerr << "lidar-scan: error: unable to get device info response"
                  << std::endl;
        status_ = false;
        return false;
    }
    ...
    std::clog << "  Firmware ver.: " << lidar_info.firmware_version
              << std::endl;
    std::clog << "  Hardware ver.: " << unsigned(lidar_info.hardware_version)
              << std::endl;
    std::clog << "  Serial number: ";
    for (int i = 0; i < 16; i++) {
        std::clog << std::setfill('0') << std::setw(2) << std::hex
...
bool RPLIDARPortGrabber::print_health() {
    ...
    if (IS_FAIL(res)) {
        std::cerr << "lidar-scan: error: unable to get device health response"
                  << std::endl;
        status_ = false;
        return false;
    }
...
bool RPLIDARPortGrabber::print_scan_modes(
    ...
    if (IS_FAIL(res)) {
        std::cerr
            << "lidar-scan: error: unable to get device scan modes response"
            << std::endl;
        return false;
    }
    ...
    for (const auto& scan_mode : scan_modes) {
        ...
        std::clog << "), "
                  ...
                  << "max distance: " << scan_mode.max_distance << "m"
                  << std::endl;
    }
    return true;
}
...
bool RPLIDARPortGrabber::launch() {
    ...
    if (IS_FAIL(res)) {
        std::cerr << "lidar-scan: error: unable to establish connection"
                  << std::endl;
        status_ = false;
        return false;
    }
    ...
    std::clog << "lidar-scan: selected scan mode: " << _u16(scan_mode_)
              << std::endl;
    ...
    std::clog << "lidar-scan: setting motor speed to [rpm]: " << rpm_
              << std::endl;
...
bool RPLIDARPortGrabber::scan() {
    ...
    if (IS_FAIL(res)) {
        std::cerr << "lidar-scan: error: unable to read scanning data"
                  << std::endl;
        status_ = false;
        return false;
    }
