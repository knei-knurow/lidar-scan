...
bool Stream::write_cloud_separator() {
    ...
    auto elapsed_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - last_cloud_)
            .count();

    status_ =
        0 <= std::fprintf(stream_, "! %zu %llu\n", ++count_, elapsed_time);
    last_cloud_ = std::chrono::steady_clock::now();
    return status_;
}

bool Stream::write_point(const PointCyl& pt_cyl) {
...
bool Stream::write_cloud(const CloudCyl& cloud_cyl) {
    ...
    for (size_t i = 0; i < cloud_cyl.size; i++) {
        ...
        status_ = 0 <= std::fprintf(stream_, "%f %f\n", cloud_cyl.pts[i].angle,
                                    cloud_cyl.pts[i].dist);
    }
    return status_;
}
